...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run biomarker discovery for linear SVM + nested CV across MOFA models."
    )
...
def run_linear_svm_nested_cv(
    ...
    for seed in seeds:
        ...
        for fold_id, (train_idx, test_idx) in enumerate(outer_cv.split(X, y), start=1):
            ...
            _ = X_test  # kept for symmetry/readability
...
def run_for_model(
    ...
    coefficients_df = run_linear_svm_nested_cv(
        X=X,
        y=y,
        seeds=seeds,
...
def main() -> None:
    ...
    base_output_dir = (
        root_dir / "data" / "biomarker_discovery" / "nested_cv" / "linear_svm"
    )
